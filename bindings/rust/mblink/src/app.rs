...
pub struct AppHandle {
    handle: mblink_sys::MBlinkHandle,
}
...
impl DevToolsHttpSession {
        ...
        loop {
            match rx.try_recv() {
                ...
                Err(mpsc::TryRecvError::Empty) => {
                    unsafe { mblink_sys::mblink_wait_events(self.handle) };
                }
    ...
    pub fn stop(self) -> Result<()> {
        check_rc_raw(unsafe { mblink_sys::mblink_devtools_http_stop(self.handle) })
    }
}
...
impl AppHandle {
    pub fn stop(self) {
        unsafe { mblink_sys::mblink_stop(self.handle) };
    }

    pub fn show(self) -> Result<()> {
        check_rc_raw(unsafe { mblink_sys::mblink_show(self.handle) })
    }

    pub fn hide(self) -> Result<()> {
        check_rc_raw(unsafe { mblink_sys::mblink_hide(self.handle) })
    }

    pub fn restore(self) -> Result<()> {
        check_rc_raw(unsafe { mblink_sys::mblink_restore(self.handle) })
    }

    pub fn set_title(self, title: &str) -> Result<()> {
        let title = to_cstring(title)?;
        check_rc_raw(unsafe { mblink_sys::mblink_set_title(self.handle, title.as_ptr()) })
    }

    pub fn set_always_on_top(self, on_top: bool) -> Result<()> {
        check_rc_raw(unsafe { mblink_sys::mblink_set_always_on_top(self.handle, on_top) })
    }
...
pub struct App {
    handle: mblink_sys::MBlinkHandle,
    shared_handles: Vec<mblink_sys::MBlinkSharedHandle>,
...
impl App {
    ...
    pub fn build(builder: AppBuilder) -> Result<Self> {
        ...
        let mut cfg = unsafe { mblink_sys::mblink_default_config() };
        cfg.title = title.as_ptr();
        builder.apply_to_raw(&mut cfg);

        let handle = unsafe { mblink_sys::mblink_create_ex(&cfg) };
        if handle.is_null() {
            return Err(Error::NullHandle);
        }
        let runtime_options = unsafe { mblink_sys::mblink_default_runtime_options() };
        let rc = unsafe { mblink_sys::mblink_configure_runtime(handle, &runtime_options) };
        if rc != 0 {
            unsafe { mblink_sys::mblink_destroy(handle) };
            check_rc_raw(rc)?;
        }
    ...
    pub fn run(&mut self) {
        ...
        unsafe { mblink_sys::mblink_run(self.handle) };
        self.cleanup_native();
    }
    ...
    fn install_default_on_close_stop(&mut self) -> Result<()> {
        ...
        let holder = Box::new(VoidHolder {
            callback: Box::new(move || unsafe {
                mblink_sys::mblink_stop(handle);
            }),
        });
        let user_data = Box::into_raw(holder);
        let rc = unsafe {
            mblink_sys::mblink_on_close(self.handle, Some(void_trampoline), user_data.cast())
        };
        if rc != 0 {
    ...
    fn cleanup_native(&mut self) {
        ...
        for control in self.control_handles.drain(..).rev() {
            unsafe {
                match control {
                    ControlHandle::LogView(handle) => mblink_sys::mblink_logview_destroy(handle),
                    ControlHandle::Terminal(handle) => mblink_sys::mblink_terminal_destroy(handle),
                }
            }
        }
        for shared in self.shared_handles.drain(..).rev() {
            unsafe { mblink_sys::mblink_shared_destroy(shared) };
        }
        for watcher in self.state_watchers.drain(..).rev() {
            unsafe {
                mblink_sys::mblink_state_unwatch(self.handle, watcher.watch_id);
                drop(Box::from_raw(watcher.user_data));
            }
        }
        unsafe { mblink_sys::mblink_destroy(self.handle) };
    ...
    pub fn poll(&self) -> Result<bool> {
        Ok(unsafe { mblink_sys::mblink_poll_events(self.handle) })
    }

    pub fn wait(&self) -> Result<bool> {
        Ok(unsafe { mblink_sys::mblink_wait_events(self.handle) })
    }

    pub fn load_html(&self, html: &str) -> Result<&Self> {
        let html = to_cstring(html)?;
        self.check_rc(unsafe { mblink_sys::mblink_load_html(self.handle, html.as_ptr()) })?;
        Ok(self)
    }

    pub fn load_html_file(&self, path: &str) -> Result<&Self> {
        let path = to_cstring(path)?;
        self.check_rc(unsafe { mblink_sys::mblink_load_html_file(self.handle, path.as_ptr()) })?;
        Ok(self)
    }
    ...
    pub fn load_entry_file(&self, path: &str, execute_html_scripts: bool) -> Result<&Self> {
        ...
        self.check_rc(unsafe {
            mblink_sys::mblink_load_entry_file(self.handle, path.as_ptr(), execute_html_scripts)
        })?;
        Ok(self)
    }
    ...
    pub fn eval_js(&self, code: &str) -> Result<&Self> {
        ...
        self.check_rc(unsafe { mblink_sys::mblink_eval_js(self.handle, code.as_ptr()) })?;
        Ok(self)
    }
    ...
    pub fn eval_module(&self, code: &str, filename: &str) -> Result<&Self> {
        ...
        self.check_rc(unsafe {
            mblink_sys::mblink_eval_module(self.handle, code.as_ptr(), filename.as_ptr())
        })?;
        Ok(self)
    }
    ...
    pub fn load_js_file(&self, path: &str) -> Result<&Self> {
        let path = to_cstring(path)?;
        self.check_rc(unsafe { mblink_sys::mblink_load_js_file(self.handle, path.as_ptr()) })?;
        Ok(self)
    }

    pub fn load_module_file(&self, path: &str) -> Result<&Self> {
        let path = to_cstring(path)?;
        self.check_rc(unsafe { mblink_sys::mblink_load_module_file(self.handle, path.as_ptr()) })?;
        Ok(self)
    }

    pub fn load_bytecode(&self, data: &[u8]) -> Result<&Self> {
        self.check_rc(unsafe {
            mblink_sys::mblink_load_bytecode(self.handle, data.as_ptr().cast(), data.len())
        })?;
        Ok(self)
    }
    ...
    pub fn mount_resource_package(
        ...
        self.check_rc(unsafe {
            mblink_sys::mblink_mount_resource_package(
                self.handle,
                package_file.as_ptr(),
    ...
    pub fn configure_runtime(&self, options: RuntimeOptions) -> Result<&Self> {
        let epoch = options
            ...
            .map(to_cstring)
            .transpose()?;
        let mut raw = unsafe { mblink_sys::mblink_default_runtime_options() };
        ...
        self.check_rc(unsafe { mblink_sys::mblink_configure_runtime(self.handle, &raw) })?;
        Ok(self)
    }

    pub fn load_embedded_runtime(&self, include_official_preact: bool) -> Result<&Self> {
        self.check_rc(unsafe {
            mblink_sys::mblink_load_embedded_runtime(self.handle, include_official_preact)
        })?;
        Ok(self)
    }

    pub fn render_frame(&self, passes: i32) -> Result<&Self> {
        self.check_rc(unsafe { mblink_sys::mblink_render_frame(self.handle, passes) })?;
        Ok(self)
    }

    pub fn runtime_epoch(&self) -> Result<String> {
        let mut out = std::ptr::null_mut();
        self.check_rc(unsafe { mblink_sys::mblink_runtime_epoch(self.handle, &mut out) })?;
        unsafe { string_from_owned_ptr(out) }
    }

    pub fn lifecycle_state(&self) -> crate::LifecycleState {
        crate::LifecycleState::from_raw(unsafe { mblink_sys::mblink_lifecycle_state(self.handle) })
    }

    pub fn lifecycle_reason(&self) -> Result<String> {
        let mut out = std::ptr::null_mut();
        self.check_rc(unsafe { mblink_sys::mblink_lifecycle_reason(self.handle, &mut out) })?;
        unsafe { string_from_owned_ptr(out) }
    }

    pub fn observe_console_json(&self) -> Result<String> {
        let mut out = std::ptr::null_mut();
        self.check_rc(unsafe { mblink_sys::mblink_observe_console_json(self.handle, &mut out) })?;
        unsafe { string_from_owned_ptr(out) }
    }

    pub fn observe_errors_json(&self) -> Result<String> {
        let mut out = std::ptr::null_mut();
        self.check_rc(unsafe { mblink_sys::mblink_observe_errors_json(self.handle, &mut out) })?;
        unsafe { string_from_owned_ptr(out) }
    }

    pub fn observe_lifecycle_json(&self) -> Result<String> {
        let mut out = std::ptr::null_mut();
        self.check_rc(unsafe { mblink_sys::mblink_observe_lifecycle_json(self.handle, &mut out) })?;
        unsafe { string_from_owned_ptr(out) }
    }
    ...
    pub fn observe_clear(&self, kind: crate::ObserveKind) -> Result<&Self> {
        self.check_rc(unsafe { mblink_sys::mblink_observe_clear(self.handle, kind.to_raw()) })?;
        Ok(self)
    }
    ...
    pub fn ui_dev_snapshot_json(&self, options: UiDevSnapshotOptions) -> Result<String> {
        ...
        let screenshot_file = options
            ...
            .map(to_cstring)
            .transpose()?;
        let mut raw = unsafe { mblink_sys::mblink_ui_dev_default_snapshot_options() };
        ...
        let mut out = std::ptr::null_mut();
        self.check_rc(unsafe {
            mblink_sys::mblink_ui_dev_snapshot_json(self.handle, &raw, &mut out)
        })?;
        unsafe { string_from_owned_ptr(out) }
    }
    ...
    pub fn ui_dev_snapshot_file(
        ...
        raw.screenshot_file = screenshot_file
            ...
            .map_or(std::ptr::null(), |v| v.as_ptr());
        self.check_rc(unsafe {
            mblink_sys::mblink_ui_dev_snapshot_file(self.handle, output_path.as_ptr(), &raw)
        })?;
        Ok(self)
    }
    ...
    pub fn ui_dev_command_json(&self, command_json: &str) -> Result<String> {
        ...
        let mut out = std::ptr::null_mut();
        self.check_rc(unsafe {
            mblink_sys::mblink_ui_dev_command_json(self.handle, command_json.as_ptr(), &mut out)
        })?;
        unsafe { string_from_owned_ptr(out) }
    }
    ...
    pub fn set_title(&self, title: &str) -> Result<&Self> {
        let title = to_cstring(title)?;
        self.check_rc(unsafe { mblink_sys::mblink_set_title(self.handle, title.as_ptr()) })?;
        Ok(self)
    }

    pub fn create_tray(&self, tooltip: &str) -> Result<&Self> {
        let tooltip = to_cstring(tooltip)?;
        self.check_rc(unsafe { mblink_sys::mblink_tray_create(self.handle, tooltip.as_ptr()) })?;
        Ok(self)
    }

    pub fn destroy_tray(&self) -> Result<&Self> {
        self.check_rc(unsafe { mblink_sys::mblink_tray_destroy(self.handle) })?;
        Ok(self)
    }

    pub fn set_tray_tooltip(&self, tooltip: &str) -> Result<&Self> {
        let tooltip = to_cstring(tooltip)?;
        self.check_rc(unsafe { mblink_sys::mblink_tray_set_tooltip(self.handle, tooltip.as_ptr()) })?;
        Ok(self)
    }

    pub fn set_tray_menu_json(&self, menu_json: &str) -> Result<&Self> {
        let menu_json = to_cstring(menu_json)?;
        self.check_rc(unsafe { mblink_sys::mblink_tray_set_menu(self.handle, menu_json.as_ptr()) })?;
        Ok(self)
    }
    ...
    pub fn set_size(&self, width: i32, height: i32) -> Result<&Self> {
        self.check_rc(unsafe { mblink_sys::mblink_set_size(self.handle, width, height) })?;
        Ok(self)
    }
    ...
    pub fn size(&self) -> Result<(i32, i32)> {
        let mut width = 0;
        let mut height = 0;
        self.check_rc(unsafe { mblink_sys::mblink_get_size(self.handle, &mut width, &mut height) })?;
        Ok((width, height))
    }

    pub fn set_position(&self, x: i32, y: i32) -> Result<&Self> {
        self.check_rc(unsafe { mblink_sys::mblink_set_position(self.handle, x, y) })?;
        Ok(self)
    }
    ...
    pub fn position(&self) -> Result<(i32, i32)> {
        let mut x = 0;
        let mut y = 0;
        self.check_rc(unsafe { mblink_sys::mblink_get_position(self.handle, &mut x, &mut y) })?;
        Ok((x, y))
    }

    pub fn set_min_size(&self, width: i32, height: i32) -> Result<&Self> {
        self.check_rc(unsafe { mblink_sys::mblink_set_min_size(self.handle, width, height) })?;
        Ok(self)
    }

    pub fn set_max_size(&self, width: i32, height: i32) -> Result<&Self> {
        self.check_rc(unsafe { mblink_sys::mblink_set_max_size(self.handle, width, height) })?;
        Ok(self)
    }

    pub fn show(&self) -> Result<&Self> {
        self.check_rc(unsafe { mblink_sys::mblink_show(self.handle) })?;
        Ok(self)
    }

    pub fn hide(&self) -> Result<&Self> {
        self.check_rc(unsafe { mblink_sys::mblink_hide(self.handle) })?;
        Ok(self)
    }

    pub fn minimize(&self) -> Result<&Self> {
        self.check_rc(unsafe { mblink_sys::mblink_minimize(self.handle) })?;
        Ok(self)
    }

    pub fn maximize(&self) -> Result<&Self> {
        self.check_rc(unsafe { mblink_sys::mblink_maximize(self.handle) })?;
        Ok(self)
    }

    pub fn restore(&self) -> Result<&Self> {
        self.check_rc(unsafe { mblink_sys::mblink_restore(self.handle) })?;
        Ok(self)
    }

    pub fn set_fullscreen(&self, fullscreen: bool) -> Result<&Self> {
        self.check_rc(unsafe { mblink_sys::mblink_set_fullscreen(self.handle, fullscreen) })?;
        Ok(self)
    }

    pub fn set_resizable(&self, resizable: bool) -> Result<&Self> {
        self.check_rc(unsafe { mblink_sys::mblink_set_resizable(self.handle, resizable) })?;
        Ok(self)
    }

    pub fn set_borderless(&self, borderless: bool) -> Result<&Self> {
        self.check_rc(unsafe { mblink_sys::mblink_set_borderless(self.handle, borderless) })?;
        Ok(self)
    }

    pub fn set_always_on_top(&self, on_top: bool) -> Result<&Self> {
        self.check_rc(unsafe { mblink_sys::mblink_set_always_on_top(self.handle, on_top) })?;
        Ok(self)
    }
    ...
    pub fn emit_str(&self, event: &str, json: &str) -> Result<()> {
        ...
        self.check_rc(unsafe { mblink_sys::mblink_emit(self.handle, event.as_ptr(), json.as_ptr()) })
    }
    ...
    pub fn unbind(&mut self, name: &str) -> Result<()> {
        let name_c = to_cstring(name)?;
        unsafe { mblink_sys::mblink_unbind(self.handle, name_c.as_ptr()) };
        ...
        F: Fn() + 'static,
    {
        ...
        let user_data = Box::into_raw(holder);
        let rc = unsafe {
            mblink_sys::mblink_tray_set_left_click_callback(
                self.handle,
        ...
        F: Fn(Value) -> Result<Value> + 'static,
    {
        ...
        let user_data = Box::into_raw(holder);
        let rc = unsafe {
            mblink_sys::mblink_tray_set_menu_callback(
                self.handle,
                Some(bind_trampoline),
                user_data.cast(),
            )
        ...
        F: Fn(i32, i32) + 'static,
    {
        ...
        let user_data = Box::into_raw(holder);
        let rc = unsafe {
            mblink_sys::mblink_on_resize(self.handle, Some(resize_trampoline), user_data.cast())
        };
        if rc != 0 {
            unsafe { drop(Box::from_raw(user_data)) };
            return self.check_rc(rc);
        ...
        F: Fn() + 'static,
    {
        ...
        let holder = Box::new(VoidHolder {
            callback: Box::new(move || {
                callback();
                unsafe { mblink_sys::mblink_stop(handle) };
            }),
        });
        let user_data = Box::into_raw(holder);
        let rc = unsafe {
        ...
        F: Fn() -> bool + 'static,
    {
        ...
        let user_data = Box::into_raw(holder);
        let rc = unsafe {
            mblink_sys::mblink_on_close_request(self.handle, Some(bool_trampoline), user_data.cast())
        };
        if rc != 0 {
            unsafe { drop(Box::from_raw(user_data)) };
            return self.check_rc(rc);
        ...
        F: Fn() + 'static,
    {
        ...
        let user_data = Box::into_raw(holder);
        let rc = unsafe {
            mblink_sys::mblink_on_focus(self.handle, Some(void_trampoline), user_data.cast())
        };
        if rc != 0 {
            unsafe { drop(Box::from_raw(user_data)) };
            return self.check_rc(rc);
        ...
        F: Fn() + 'static,
    {
        ...
        let user_data = Box::into_raw(holder);
        let rc = unsafe {
            mblink_sys::mblink_on_blur(self.handle, Some(void_trampoline), user_data.cast())
        };
        if rc != 0 {
            unsafe { drop(Box::from_raw(user_data)) };
            return self.check_rc(rc);
        ...
        F: Fn(f32) + 'static,
    {
        ...
        let user_data = Box::into_raw(holder);
        let rc = unsafe {
            mblink_sys::mblink_on_update(self.handle, Some(update_trampoline), user_data.cast())
        };
        if rc != 0 {
            unsafe { drop(Box::from_raw(user_data)) };
            return self.check_rc(rc);
    ...
    pub fn shared(&mut self, name: &str) -> Result<Shared<'_>> {
        ...
        let handle = unsafe { mblink_sys::mblink_shared_create(self.handle, name.as_ptr()) };
        if handle.is_null() {
            return Err(Error::NullHandle);
        }
    ...
    pub fn logview(&mut self, element_id: &str) -> Result<LogView<'_>> {
        ...
        let handle = unsafe { mblink_sys::mblink_logview_get(self.handle, element_id.as_ptr()) };
    ...
    pub fn terminal(&mut self, element_id: &str) -> Result<Terminal<'_>> {
        let element_id = to_cstring(element_id)?;
        let handle = unsafe { mblink_sys::mblink_terminal_get(self.handle, element_id.as_ptr()) };
    ...
    pub(crate) fn unregister_state_watch(&mut self, watch_id: i32) {
            ...
            .position(|entry| entry.watch_id == watch_id)
        {
            ...
            unsafe {
                mblink_sys::mblink_state_unwatch(self.handle, watch_id);
                drop(Box::from_raw(watcher.user_data));
            }
        }
    }
    ...
    pub fn version() -> Result<String> {
        ensure_init();
        unsafe { string_from_const_ptr(mblink_sys::mblink_version()) }
    }

    pub fn devtools_open(&self) -> Result<&Self> {
        self.check_rc(unsafe { mblink_sys::mblink_devtools_open(self.handle) })?;
        Ok(self)
    }

    pub fn devtools_close(&self) -> Result<&Self> {
        self.check_rc(unsafe { mblink_sys::mblink_devtools_close(self.handle) })?;
        Ok(self)
    }
    ...
    pub fn devtools_http_session(
        ...
        let mut raw = unsafe { mblink_sys::mblink_devtools_default_http_options() };
        ...
        let mut info = mblink_sys::MBlinkDevToolsHttpInfo {
            port: 0,
            url: std::ptr::null_mut(),
            auth_token: std::ptr::null_mut(),
        };
        self.check_rc(unsafe {
            mblink_sys::mblink_devtools_http_start(self.handle, &raw, &mut info)
        })?;
        let url = unsafe { string_from_raw_http_field(info.url) }?;
        let token = unsafe { string_from_raw_http_field(info.auth_token) }?;
        let port = info.port;
        unsafe { mblink_sys::mblink_devtools_http_info_free(&mut info) };
    ...
    pub fn devtools_http_stop(&self) -> Result<&Self> {
        self.check_rc(unsafe { mblink_sys::mblink_devtools_http_stop(self.handle) })?;
        Ok(self)
    }

    pub(crate) fn raw_handle(&self) -> mblink_sys::MBlinkHandle {
        self.handle
    }
...
fn ensure_init() {
    INIT.call_once(|| {
        let _ = unsafe { mblink_sys::mblink_init() };
    });
}
...
pub(crate) fn check_rc_raw(rc: i32) -> Result<()> {
    ...
    let message = unsafe { string_from_const_ptr(mblink_sys::mblink_last_error()) }
        .unwrap_or_else(|_| "unknown MBlink error".to_string());
    Err(Error::Mblink { code: rc, message })
}
...
fn post_jsonrpc(url: &str, auth_token: &str, payload: &Value) -> Result<Value> {
    ...
    if !auth_token.is_empty() {
        request.push_str("X-MBLINK-DevTools-Token: ");
