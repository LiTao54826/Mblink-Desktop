// Preact Todo App - 纯 JS 版本（非 ES 模块，用于 Python 绑定测试）
// 使用全局变量方式访问 Preact
...
function TodoItem({ todo, onToggle, onDelete }) {
    ...
    return h('div', { className: 'todo-item' },
        h('input', {
            ...
            onChange: () => {
                console.log(`[TodoItem] Checkbox clicked for todo ${todo.id}, current=${todo.completed}`);
                onToggle(todo.id);
            }
        }),
        h('div', {
...
function App() {
    ...
    const handleToggle = (id) => {
        console.log('[App] handleToggle called for id:', id);
        const newTodos = todos.map(todo => {
            if (todo.id === id) {
                console.log(`[App] Toggling todo ${id}: ${todo.completed} -> ${!todo.completed}`);
                return { ...todo, completed: !todo.completed };
            }
            return todo;
        });
        console.log('[App] Setting new todos:', JSON.stringify(newTodos));
        setTodos(newTodos);
    };
    ...
    console.log('[App] Rendering, todos:', JSON.stringify(todos));
