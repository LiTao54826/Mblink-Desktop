...
class SharedState:
    ...
    def __setattr__(self, key, value):
        ...
        if value is None:
            lib.mblink_shared_set_null(sh, k)
        elif isinstance(value, bool):
            lib.mblink_shared_set_bool(sh, k, value)
        elif isinstance(value, int):
            lib.mblink_shared_set_int(sh, k, value)
        elif isinstance(value, float):
            lib.mblink_shared_set_double(sh, k, value)
        elif isinstance(value, str):
            lib.mblink_shared_set_string(sh, k, value.encode('utf-8'))
        else:
            # list, dict 等复杂类型 → JSON
            lib.mblink_shared_set_json(sh, k,
    ...
    def __getattr__(self, key):
        ...
        t = lib.mblink_shared_get_type(sh, k)

        if t == _TYPE_NULL:
            return None
        elif t == _TYPE_BOOL:
            return lib.mblink_shared_get_bool(sh, k)
        elif t == _TYPE_INT:
            return lib.mblink_shared_get_int(sh, k)
        elif t == _TYPE_DOUBLE:
            return lib.mblink_shared_get_double(sh, k)
        elif t == _TYPE_STRING:
            raw = lib.mblink_shared_get_string(sh, k)
            if raw:
                try:
            ...
            raw = lib.mblink_shared_get_json(sh, k)
            if raw:
                try:
    ...
    def __delattr__(self, key):
        lib = object.__getattribute__(self, '_lib')
        sh = object.__getattribute__(self, '_shared')
        lib.mblink_shared_delete(sh, key.encode('utf-8'))

    def __contains__(self, key):
        lib = object.__getattribute__(self, '_lib')
        sh = object.__getattribute__(self, '_shared')
        return lib.mblink_shared_has(sh, key.encode('utf-8'))
    ...
    def batch(self):
        ...
        lib = object.__getattribute__(self, '_lib')
        sh = object.__getattribute__(self, '_shared')
        lib.mblink_shared_batch_begin(sh)
        try:
            yield self
        finally:
            lib.mblink_shared_batch_end(sh)
