...
func main() {
	base := filepath.Join(os.TempDir(), "mblink-go-resource-mount-example")
	...
	html := `<!doctype html><html><body style="font-family:sans-serif;padding:24px;"><h1 id="title">MBlink Go Resource Mount</h1><p id="desc">This page is loaded from a compiled resource package.</p></body></html>`
	must(os.WriteFile(filepath.Join(inputDir, "index.html"), []byte(html), 0o644))
	must(mblink.CompileResources(inputDir, packageFile, ""))

	app, err := autorun.NewApp("MBlink Go Resources", 800, 600)
	if err != nil {
		log.Fatal(err)
	}
	defer app.Close()
	...
	must(autorun.Run(app, func(tick int) error {
		...
		if !strings.Contains(report, "MBlink Go Resource Mount") {
			return fmt.Errorf("unexpected mounted content: %s", report)
		}
		app.Stop()
		return nil
