...
static inline unsigned long mblink_go_current_thread_id(void) {
...
type App struct {
	handle          C.MBlinkHandle
	sharedHandles   []C.MBlinkSharedHandle
...
func currentThreadID() uint32 {
	return uint32(C.mblink_go_current_thread_id())
}

func Version() string {
	ensureInit()
	return C.GoString(C.mblink_version())
}

func DefaultConfig() Config {
    ensureInit()
    raw := C.mblink_default_config()
	return Config{
...
func NewWithConfig(cfg Config) (*App, error) {
	...
	h := C.mblink_create_ex(&raw)
	if h == nil {
		runtime.UnlockOSThread()
		return nil, newError(-1, lastErrorMessage())
	}
	runtimeOptions := C.mblink_default_runtime_options()
	if err := checkRC(C.mblink_configure_runtime(h, &runtimeOptions)); err != nil {
		C.mblink_destroy(h)
		runtime.UnlockOSThread()
		return nil, err
	}
...
func (h AppHandle) Stop()          { C.mblink_stop(h.handle) }
func (a *App) Stop()               { C.mblink_stop(a.handle) }
func (a *App) Run()                { C.mblink_run(a.handle) }
func (a *App) Poll() (bool, error) { return bool(C.mblink_poll_events(a.handle)), nil }
func (a *App) Wait() (bool, error) { return bool(C.mblink_wait_events(a.handle)), nil }
...
func (a *App) Close() {
	...
	for _, c := range a.controls {
		if c.kind == "logview" {
			C.mblink_logview_destroy((C.MBlinkLogViewHandle)(c.handle))
		} else {
			C.mblink_terminal_destroy((C.MBlinkTerminalHandle)(c.handle))
		}
	}
	for _, h := range a.sharedHandles {
		C.mblink_shared_destroy(h)
	}
