...
func (a *App) LoadHTML(html string) error {
	v, done := cString(html)
	defer done()
	return checkRC(C.mbink_load_html(a.handle, v))
}
...
func (a *App) EvalJS(code string) error {
	v, done := cString(code)
	defer done()
	return checkRC(C.mbink_eval_js(a.handle, v))
}
func (a *App) EvalModule(code, filename string) error {
	...
	defer fd()
	return checkRC(C.mbink_eval_module(a.handle, cv, fv))
}
...
func (a *App) LoadBytecode(data []byte) error {
	...
	return checkRC(C.mbink_load_bytecode(a.handle, unsafe.Pointer(&data[0]), C.size_t(len(data))))
}
...
func (a *App) SetTitle(title string) error {
	...
	defer done()
	return checkRC(C.mbink_set_title(a.handle, v))
}
func (a *App) CreateTray(tooltip string) error {
	v, done := cString(tooltip)
	defer done()
	return checkRC(C.mbink_tray_create(a.handle, v))
}
func (a *App) DestroyTray() error { return checkRC(C.mbink_tray_destroy(a.handle)) }
func (a *App) SetTrayTooltip(tooltip string) error {
	v, done := cString(tooltip)
	defer done()
	return checkRC(C.mbink_tray_set_tooltip(a.handle, v))
}
func (a *App) SetTrayMenuJSON(menuJSON string) error {
	v, done := cString(menuJSON)
	defer done()
	return checkRC(C.mbink_tray_set_menu(a.handle, v))
}
...
func (a *App) Emit(event string, value any) error {
	...
	defer jd()
	return checkRC(C.mbink_emit(a.handle, ev, jv))
}
func (a *App) SetSize(width, height int) error {
	return checkRC(C.mbink_set_size(a.handle, C.int(width), C.int(height)))
}
func (a *App) Size() (int, int, error) {
	var w, h C.int
	if err := checkRC(C.mbink_get_size(a.handle, &w, &h)); err != nil {
		return 0, 0, err
	}
	return int(w), int(h), nil
}
func (a *App) SetPosition(x, y int) error {
	return checkRC(C.mbink_set_position(a.handle, C.int(x), C.int(y)))
}
func (a *App) Position() (int, int, error) {
	var x, y C.int
	if err := checkRC(C.mbink_get_position(a.handle, &x, &y)); err != nil {
		return 0, 0, err
	}
	return int(x), int(y), nil
}
func (a *App) SetMinSize(width, height int) error {
	return checkRC(C.mbink_set_min_size(a.handle, C.int(width), C.int(height)))
}
func (a *App) SetMaxSize(width, height int) error {
	return checkRC(C.mbink_set_max_size(a.handle, C.int(width), C.int(height)))
}
func (a *App) Show() error     { return checkRC(C.mbink_show(a.handle)) }
func (a *App) Hide() error     { return checkRC(C.mbink_hide(a.handle)) }
func (a *App) Minimize() error { return checkRC(C.mbink_minimize(a.handle)) }
func (a *App) Maximize() error { return checkRC(C.mbink_maximize(a.handle)) }
func (a *App) Restore() error  { return checkRC(C.mbink_restore(a.handle)) }
func (a *App) SetFullscreen(v bool) error {
	return checkRC(C.mbink_set_fullscreen(a.handle, boolToC(v)))
}
func (a *App) SetResizable(v bool) error { return checkRC(C.mbink_set_resizable(a.handle, boolToC(v))) }
func (a *App) SetBorderless(v bool) error {
	return checkRC(C.mbink_set_borderless(a.handle, boolToC(v)))
}
func (a *App) SetAlwaysOnTop(v bool) error {
	return checkRC(C.mbink_set_always_on_top(a.handle, boolToC(v)))
}
func (h AppHandle) Show() error    { return checkRC(C.mbink_show(h.handle)) }
func (h AppHandle) Hide() error    { return checkRC(C.mbink_hide(h.handle)) }
func (h AppHandle) Restore() error { return checkRC(C.mbink_restore(h.handle)) }
func (h AppHandle) SetTitle(title string) error {
	v, d := cString(title)
	defer d()
	return checkRC(C.mbink_set_title(h.handle, v))
}
func (h AppHandle) SetAlwaysOnTop(v bool) error {
	return checkRC(C.mbink_set_always_on_top(h.handle, boolToC(v)))
}
