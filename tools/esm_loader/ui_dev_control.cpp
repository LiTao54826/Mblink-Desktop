...
const devClick = (el) => {
  ...
  if (isCheckable && !!el.checked === beforeChecked) {
    ...
    if (type == "query_element") {
        ...
        js << "const el = q(); if (!el) { if (!performAction) return stalePostAction(); throw new Error('element not found: ' + selector); } const x = " << (cmd.contains("x") ? JsonLiteral(cmd["x"]) : std::string("null")) << "; const y = " << (cmd.contains("y") ? JsonLiteral(cmd["y"]) : std::string("null")) << "; if (performAction) { if (x !== null) el.scrollLeft = x; if (y !== null) el.scrollTop = y; } return { selector, scroll: scrollOf(el), element: summaryOf(el, false), post_action_found: true };";
    } else if (type == "highlight") {
        js << "const el = q(); if (!el) { if (!performAction) return stalePostAction(); throw new Error('element not found: ' + selector); } const color = " << JsonLiteral(cmd.value("color", std::string("#ff4d4f"))) << "; const previous = el.style && el.style.outline ? String(el.style.outline) : ''; if (performAction && el.style) el.style.outline = '2px solid ' + color; return performAction ? { selector, highlighted: true, color, previous_outline: previous, element: summaryOf(el, false), post_action_found: true } : { selector, highlighted: true, color, element: summaryOf(el, false), post_action_found: true };";
    }
    js << R"JS(
})())JS";
    return js.str();
