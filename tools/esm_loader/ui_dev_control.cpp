...
const pickStyle = (el) => { const s = getComputedStyle(el); return { display: s.display || '', position: s.position || '', width: s.width || '', height: s.height || '', color: s.color || '', background_color: s.backgroundColor || '', opacity: s.opacity || '', overflow_x: s.overflowX || '', overflow_y: s.overflowY || '', z_index: s.zIndex || '', flex: s.flex || '' }; };
    ...
    if (type == "query_element") {
        ...
        js << "const el = q(); if (!el) throw new Error('element not found: ' + selector); if (performAction) el.click(); return { selector, clicked: true, element: summaryOf(el, false) };";
    } else if (type == "input_text") {
        js << "const el = q(); if (!el) throw new Error('element not found: ' + selector); if (el.value === undefined) throw new Error('target does not support value: ' + selector); if (performAction) { const nextValue = " << JsonLiteral(cmd.value("text", std::string{})) << "; if (el.focus) el.focus(); el.value = nextValue; if (typeof Event === 'function') { try { el.dispatchEvent(new Event('input')); } catch (_) {} try { el.dispatchEvent(new Event('change')); } catch (_) {} } } return { selector, value: el.value === undefined ? null : el.value, element: summaryOf(el, false) };";
    } else if (type == "scroll") {
        js << "const el = q(); if (!el) throw new Error('element not found: ' + selector); const x = " << (cmd.contains("x") ? JsonLiteral(cmd["x"]) : std::string("null")) << "; const y = " << (cmd.contains("y") ? JsonLiteral(cmd["y"]) : std::string("null")) << "; if (performAction) { if (x !== null) el.scrollLeft = x; if (y !== null) el.scrollTop = y; } return { selector, scroll: scrollOf(el), element: summaryOf(el, false) };";
    } else if (type == "highlight") {
        js << "const el = q(); if (!el) throw new Error('element not found: ' + selector); const color = " << JsonLiteral(cmd.value("color", std::string("#ff4d4f"))) << "; const previous = el.style && el.style.outline ? String(el.style.outline) : ''; if (performAction && el.style) el.style.outline = '2px solid ' + color; return performAction ? { selector, highlighted: true, color, previous_outline: previous, element: summaryOf(el, false) } : { selector, highlighted: true, color, element: summaryOf(el, false) };";
    }
    js << R"JS(
})())JS";
    return js.str();
