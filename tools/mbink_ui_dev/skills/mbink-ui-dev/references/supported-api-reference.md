# MBink UI Supported API Reference

Use this file when an agent is unsure whether a JavaScript, DOM, Preact, host bridge, or MBink native-element API is supported by the current MBink UI runtime.

This is a conservative compatibility list. Prefer APIs listed as supported. If an API is not listed here or in the linked references, add it only in a small isolated slice and verify it with `mbink-ui-dev build`, `snapshot`, `query_element`, `inspect`, `logs`, and `errors`.
...
| `document.createElement(tag)` | supported | Includes standard tags plus MBink native tags such as `terminal` and `logview`. |
...
| full browser navigation, history, downloads, `target="_blank"` | unsupported contract | MBink UI is a desktop runtime, not a browser. |
| native form submission | unsupported contract | Handle form submission in JS and through `hostApi`. |
| Shadow DOM, custom elements | unsupported contract | MBink native tags are not Web Components. |
...
2. Build with `mbink-ui-dev build`.
