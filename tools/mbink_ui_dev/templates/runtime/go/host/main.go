...
func main() {
	...
	title := firstNonEmpty(config.Window.Title, config.Name, "MBink")
	...
	appConfig := mbink.DefaultConfig().
		WithTitle(title).
		WithSize(width, height).
		WithBorderless(config.Window.Borderless).
		WithResizable(resizable)

	app, err := mbink.NewWithConfig(appConfig)
	if err != nil {
		log.Fatal(err)
	}
	defer app.Close()
...
func findProjectRoot(start string) string {
	...
	for {
		if exists(filepath.Join(current, "mbink.config.json")) ||
...
func resolveResourcePackage(root string) string {
	...
	if isValidEmbeddedPackage(embeddedResourcePackage) {
		path := filepath.Join(os.TempDir(), "mbink-template-go", executableStem(), "app.mbrp")
