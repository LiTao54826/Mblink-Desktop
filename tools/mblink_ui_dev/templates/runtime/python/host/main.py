...
EMBEDDED_CONFIG = {
    ...
    "purpose": "minimal",
    "runtime": "python",
    "window": {"title": "MBlink", "width": 900, "height": 640, "resizable": True},
}
RESOURCE_CONFIG_PATH = "app/mblink.config.json"
...
LOCAL_DLL_DIR = LOCAL_BINDING_ROOT / "mblink" / "bin"
FROZEN_ROOT = pyinstaller_base_dir()
FROZEN_DLL_DIR = FROZEN_ROOT / "mblink" / "bin"

if LOCAL_BINDING_ROOT.exists():
    sys.path.insert(0, str(LOCAL_BINDING_ROOT))
    if (LOCAL_DLL_DIR / "mblink.dll").exists():
        os.environ.setdefault("MBLINK_DLL_PATH", str(LOCAL_DLL_DIR))
if getattr(sys, "frozen", False):
    sys.path.insert(0, str(FROZEN_ROOT))
    if (FROZEN_DLL_DIR / "mblink.dll").exists():
        os.environ.setdefault("MBLINK_DLL_PATH", str(FROZEN_DLL_DIR))

try:
    from mblink import App
    from mblink import load_resource_file
except ImportError as exc:
    raise SystemExit(
        "Cannot import the MBlink Python binding. Run mblink-ui-dev build so the local binding is packaged."
    ) from exc
...
def load_config(resource_package):
    config_path = PROJECT_ROOT / "mblink.config.json"
...
def main():
    ...
    title = window.get("title") or config.get("name") or "MBlink"
