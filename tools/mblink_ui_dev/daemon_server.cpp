...
bool ShouldWatchFile(const std::filesystem::path& project_root,
    ...
    if (lower == "mblink.config.json") return true;
...
std::string SanitizeArtifactStem(std::string value) {
    ...
    return value.empty() ? std::string{"mblink-app"} : value;
}
...
nlohmann::json CompileResourcePackageStatus(const std::filesystem::path& project_root,
    ...
    if (std::filesystem::exists(config_path, ec)) {
        std::string ignored;
        CopyFileOverwrite(config_path, input_dir / "mblink.config.json", &ignored);
    }

    const auto input_dir_text = PathToUtf8(input_dir);
    const auto output_file_text = PathToUtf8(output_file);
    std::string compile_error;
    if (!mblink::resourcepkg::CompileResources(input_dir_text.c_str(), output_file_text.c_str(), "", compile_error)) {
...
std::filesystem::path FindMblinkRuntimeLibrary(const std::filesystem::path& project_root) {
    const auto exe_dir = GetCurrentExecutablePath().parent_path();
    const std::vector<std::filesystem::path> candidates = {
        exe_dir / "mblink.dll",
        project_root / "rust_host" / "vendor" / "mblink-sys" / "runtime" / "mblink.dll",
        project_root / ".mblink" / "mblink-go" / "mblink.dll",
        project_root / "vendor" / "mblink-go" / "mblink.dll",
        project_root / "vendor" / "mblink" / "bin" / "mblink.dll"
    };
    for (const auto& candidate : candidates) {
        std::error_code ec;
        if (std::filesystem::exists(candidate, ec) && !std::filesystem::is_directory(candidate, ec)) {
...
bool CopyRuntimeDllToFinal(const std::filesystem::path& project_root,
                           ...
                           std::string* error) {
    const auto runtime = FindMblinkRuntimeLibrary(project_root);
    if (runtime.empty()) {
        if (error) *error = "mblink.dll not found for final artifact";
        return false;
    }
...
nlohmann::json BuildRustHostArtifact(const std::filesystem::path& project_root,
    ...
    const auto built_exe = project_root / "rust_host" / "target" / "release" / "mblink-template-rust.exe";
    ...
    if (!CopyRuntimeDllToFinal(project_root, final_dir, &status["outputs"], error)) {
        status["ok"] = false;
        status["error"] = error ? *error : "failed to copy mblink.dll";
        return status;
    }
...
nlohmann::json BuildPythonHostArtifact(const std::filesystem::path& project_root,
    ...
    const auto dll_path = project_root / "vendor" / "mblink" / "bin" / "mblink.dll";
    if (!std::filesystem::exists(dll_path)) {
        if (error) *error = "vendor/mblink/bin/mblink.dll not found";
        return nlohmann::json{{"ok", false}, {"runtime", "python"}, {"error", error ? *error : ""}};
    }
    ...
    pyinstaller_args +=
        ...
        " --add-binary " + QuoteForCmd(PathToUtf8(dll_path) + ";mblink/bin") +
