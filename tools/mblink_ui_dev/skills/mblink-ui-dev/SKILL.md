---
name: mblink-ui-dev
description: Operate the MBlink UI development toolchain through `mblink-ui-dev` CLI/MCP and the optional `mblink_devtools.dll` runtime HTTP MCP endpoint to initialize projects, open runtimes, inspect live snapshots/screenshots, query and inspect UI elements, click or input or scroll or highlight live UI, read/write project files, build/watch/reload, diagnose console or JS errors, and verify C API parity across `esm_loader`, Python, Rust, and Go bindings. Use when Codex needs to create, debug, test, or iterate on an MBlink UI project, when a user mentions `mblink-ui-dev`, MBlink UI snapshots, daemon/watch workflows, selector-based UI diagnosis, `mblink_devtools.dll`, runtime HTTP MCP, C API host parity, or MCP access from Claude Desktop/Cursor.
---
...
Use `mblink-ui-dev` as the primary project development surface for MBlink UI. Treat the current architecture as split:

- `mblink-ui-dev` owns project scaffolding, build/watch, managed runtime open/reload, files, logs, errors, snapshots, and stdio MCP.
- `mblink.dll` owns the runtime core and public host behavior through the C API.
- `mblink_devtools.dll` is an optional development plugin loaded on demand for live UI snapshot/control and local Streamable HTTP MCP from C API hosts.
- `esm_loader`, Python, Rust, Go, and future bindings should expose the same observable behavior through `core/api/mblink.h` and `core/devtools/mblink_devtools.h`, not through language-specific shortcuts.
...
- MBlink UI is primarily a desktop-app framework. Make the outermost `body` or root shell fill the full window by default, keep `body` non-scrollable unless page-level scrolling is genuinely required, and place scrollbars inside the content regions that need them.
...
- Use `mblink-ui-dev serve` stdio MCP when the agent benefits from structured tools, resources, session-scoped `active_project`, or IDE integration for project work.
- Use the runtime HTTP MCP endpoint only when a C API host explicitly enables `mblink_devtools.dll` for live UI analysis/control in `esm_loader`, Python, Rust, or Go.
...
1. Open the project through `mblink-ui-dev open` or MCP `open_project`.
...
- Build the UI with realistic mock data first. Finish layout, state, loading, empty, error, and interaction behavior in `mblink-ui-dev` before wiring real host behavior.
...
- After the UI validation gate passes, implement the narrow host API surface behind `ui/bridge.js`, then validate the real Python, Rust, or Go host runtime and finish with `mblink-ui-dev build`.
...
- If a host-language feature needs runtime support that is missing, add the narrow capability to `core/api/mblink.h` or the optional devtools plugin API in `core/devtools/mblink_devtools.h`, then update the bindings. Do not solve parity-critical behavior only inside `mblink-ui-dev`.
...
- Use `mblink_devtools.dll` only as a development companion. It is shipped next to `mblink-ui-dev.exe` and `esm_loader.exe`, and host bindings load it on demand from `MBLINK_DEVTOOLS_PATH`, the `mblink.dll` directory, or the process search path. Generated host packages should not vendor it.
...
- Use `mblink-ui-dev build` for the shared UI and generated artifact, then run the host entrypoint for the runtime you are changing.
...
- If the host exposes `mblink_devtools.dll`, use `ui_dev_snapshot`, `ui_dev_command`, or `devtools_http_session` so the loop stays on the same C API/devtools path.
...
- Current MBlink runtimes dispatch host shared writes through `globalThis.__mblinkSharedUpdateDispatcher(changedKeys)`. If you wrap this dispatcher, call the original dispatcher first so MBlink's root/dependency tracking can still run.
...
function installSharedUpdateBridge(forceUpdate, rootName = 'data') {
  const originalDispatcher = globalThis.__mblinkSharedUpdateDispatcher;
  let pending = false;
  let disposed = false;
  ...
  globalThis.__mblinkSharedUpdateDispatcher = dispatcher;
  return () => {
    disposed = true;
    if (globalThis.__mblinkSharedUpdateDispatcher === dispatcher) {
      globalThis.__mblinkSharedUpdateDispatcher = originalDispatcher;
    }
  };
}
...
- When a visual issue clearly contradicts valid CSS or expected MBlink behavior, diagnose and fix the MBlink framework first instead of hiding the problem with project-specific CSS workarounds. Temporary UI-side workarounds are acceptable only to isolate the failure, and should be removed after the framework fix lands.
- Classify the issue before fixing: app CSS mistakes belong in the app or example; missing snapshot/control behavior belongs in `mblink_devtools.dll` or the UI-dev command path; runtime behavior that must match across `esm_loader`, Python, Rust, and Go belongs behind the shared C API/devtools C API plus binding updates; valid CSS/DOM/rendering failures belong in the framework with a regression test.
...
## Author Within Current MBlink Limits

- Treat MBlink as a QuickJS plus MBlink DOM desktop runtime, not as Chromium, Firefox, Safari, Node.js, or a complete Web platform.
...
- Prefer the template pattern: `import { h, render } from 'preact'` and author components with `h(...)`. If using JSX in `.js`, `.mjs`, `.jsx`, `.ts`, or `.tsx`, keep `h` and `Fragment` in scope unless `mblink.config.json` intentionally changes the JSX settings and the build is verified.
...
- Do not rely on TypeScript type checking from `mblink-ui-dev build`; current TypeScript and TSX handling is esbuild transpilation only.
...
- Use MBlink's native `<terminal>` and `<logview>` elements for terminal emulation and high-volume log display instead of browser or npm terminal/log widgets. Read [native-elements.md](references/native-elements.md) before using either element.
...
- CLI `mblink-ui-dev init` now accepts an optional target directory. `mblink-ui-dev init` scaffolds into the current directory, while `mblink-ui-dev init my-app` scaffolds into `my-app`.
...
- Templates are embedded into `mblink-ui-dev.exe` at build time, so distribution does not depend on shipping a separate `tools/mblink_ui_dev/templates` directory.
- Distribute `mblink-ui-dev.exe` with adjacent `mblink.dll` and development-only `mblink_devtools.dll` on Windows. Host-runtime init copies only `mblink.dll` into generated Python, Go, or Rust projects; `mblink_devtools.dll` remains a local development companion loaded on demand.
- Runtime templates vendor their MBlink binding files, and init copies the runtime library, so generated Python, Go, and Rust projects are self-contained and do not depend on `MBLINK_REPO_ROOT`.
- Use `mblink-ui-dev build` as the release path. It compiles the UI into `app.mbrp` and then builds the selected host artifact in one command.
...
- Use `mblink-ui-dev serve` stdio MCP by default for project-scoped work. Use the runtime HTTP MCP endpoint only when a C API host explicitly enables `mblink_devtools.dll` for live UI analysis/control. Treat MCP notifications, additional ecosystem integrations, and VS Code side-panel preview as future-stage items, not current assumptions.
