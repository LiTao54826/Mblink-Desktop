# MBlink UI Compatibility Guidelines

Use this reference when writing MBlink UI code, choosing dependencies, changing scaffold templates, or diagnosing code that works in a browser but fails or renders blank in MBlink.

## Runtime Model

- MBlink UI runs on QuickJS plus MBlink DOM, layout, render, networking, and host bindings. It is not Chromium, WebView2, Node.js, or a full browser.
- `mblink.dll` is the runtime core. `mblink-ui-dev`, `esm_loader`, Python, Rust, and Go should share observable behavior through `core/api/mblink.h`; development-only snapshot/control and HTTP MCP behavior should flow through the optional `core/devtools/mblink_devtools.h` plugin API.
...
- Verify behavior with `mblink-ui-dev open`, `build`, `snapshot`, `query_element`, `inspect`, `logs`, and `errors`. Do not assume browser-tested code is compatible.
...
- Build the UI first with `mblink-ui-dev`, shared `ui/bridge.js`, and realistic mock data. Do not begin Python, Rust, or Go feature implementation until the UI has passed dev validation.
...
- After the UI gate passes, wire the real host through the same `hostApi` names used by the mock bridge, validate the host runtime, then use `mblink-ui-dev build` for the final artifact.
...
- Use `mblink_devtools.dll` as a development companion only. It may sit next to `mblink-ui-dev.exe`, `esm_loader.exe`, and `mblink.dll`; generated host projects and binding packages should not vendor it.
- Bindings should load `mblink_devtools.dll` only when UI-dev snapshot/control, DevTools panel, or HTTP MCP features are used. Resolution should prefer explicit path or `MBLINK_DEVTOOLS_PATH`, then the runtime DLL directory or process search path.
- When a parity issue appears, compare `mblink-ui-dev` snapshot/control output, `esm_loader` UI-dev files, and the target binding's UI-dev helper before changing app CSS or host-specific wrapper code.
...
- Do not write a full complex screen or large component before the first runtime check. In MBlink, delayed validation makes framework, DOM, CSS, and runtime compatibility issues much harder to isolate.
...
- The default build uses classic JSX settings: `jsx_factory = "h"` and `jsx_fragment = "Fragment"`. If authoring JSX, keep those identifiers in scope or explicitly change and verify `mblink.config.json`.
...
- `mblink-ui-dev build` is the release path. It compiles the UI to `.dist/App.js`, packages `.dist/app.mbrp`, and then builds the selected host artifact.
...
- TypeScript is transpiled only. Do not rely on `mblink-ui-dev build` for type checking, declaration generation, decorators, custom transformers, or path aliases that are not already handled by esbuild and the project configuration.
...
Do not replace these with browser or npm widgets such as xterm.js-style terminal canvases or virtualized log viewers unless the project has a verified reason. These tags are MBlink runtime elements, not standard HTML and not Web Components; their supported JavaScript methods are the methods documented in the native-elements reference.
...
- Avoid browser-specific layout assumptions and advanced CSS features unless verified in MBlink: container queries, complex filters/backdrop filters, advanced animations, sticky edge cases, custom scrollbars, complex writing modes, and print/media-query workflows.
...
- `python`, `rust`, and `go` templates provide host adapters and receive a copied MBlink runtime library during init on Windows.
...
- Prefer `mblink-ui-dev build` over manually running `cargo`, `go build`, or `PyInstaller` for release artifacts. Manual host commands are only for targeted binding validation.
- Final Windows output differs by runtime:
  - Rust and Go produce an exe plus `mblink.dll`.
  - Python uses PyInstaller one-file output and embeds the MBlink DLL plus resources.
  - Tool runtime produces the UI resource package and is not a standalone host app.
- Host-runtime builds read `build.icon` from `mblink.config.json` and apply it only to the final Windows `.exe` resource icon. This does not change window, tray, or in-app icons.
...
- Do not hard-code absolute repository paths, local machine paths, or `MBLINK_REPO_ROOT` in generated projects.
- Do not duplicate `mblink.dll` inside each embedded template. The CLI distribution should include one adjacent runtime DLL, and host init copies it into the generated project. `mblink_devtools.dll` is a development companion shipped next to `mblink-ui-dev.exe`, `esm_loader.exe`, and `mblink.dll`; generated Python, Go, and Rust host packages should not vendor it.
...
- `mblink-ui-dev serve` uses stdio MCP for project-level work. The optional `mblink_devtools.dll` runtime HTTP MCP endpoint is local-only and limited to live UI analysis/control for an already-running C API host. It does not provide init, build, watch, files, logs, errors, daemon state, or project resources. Push notifications and VS Code side-panel preview are future work.
...
1. Build with `mblink-ui-dev build`.
