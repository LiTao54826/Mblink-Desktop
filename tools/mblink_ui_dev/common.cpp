...
bool LooksLikeMblinkRepoRoot(const std::filesystem::path& path) {
    std::error_code ec;
...
std::optional<std::filesystem::path> FindMblinkRepoRootUpwards(std::filesystem::path start) {
    std::error_code ec;
    ...
    while (!start.empty()) {
        if (LooksLikeMblinkRepoRoot(start)) return start;
...
bool CopyRuntimeLibraryFromToolDirectory(const std::filesystem::path& output_path,
    ...
    if (!std::filesystem::exists(source, ec) || std::filesystem::is_directory(source, ec)) {
        if (error) {
            *error = "MBlink runtime library not found next to mblink-ui-dev: " + source.string();
        }
        return false;
    }
    ...
    if (ec) {
        if (error) *error = "failed to copy MBlink runtime library: " + ec.message();
        return false;
    }
    return true;
}
...
std::vector<std::filesystem::path> RuntimeLibraryTargetsForProject(const std::filesystem::path& root,
    ...
    if (runtime == "rust") {
        return {root / "rust_host" / "vendor" / "mblink-sys" / "runtime" / file_name};
    }
    if (runtime == "go") {
        return {root / ".mblink" / "mblink-go" / file_name};
    }
    if (runtime == "python") {
        return {root / "vendor" / "mblink" / "bin" / file_name};
    }
    return {};
}
...
std::filesystem::path ResolveMblinkRepoRoot(const std::filesystem::path& project_root) {
    ...
    for (const auto& candidate : candidates) {
        if (const auto found = FindMblinkRepoRootUpwards(candidate); found.has_value()) return *found;
    }
    return {};
}
...
std::string ApplyEmbeddedTemplateVariables(std::string content,
    ...
    const auto repo_root = ResolveMblinkRepoRoot(project_root);
    const auto repo_root_value = repo_root.empty() ? std::string{} : repo_root.generic_string();
    ReplaceAll(content, "{{MBLINK_REPO_ROOT}}", repo_root_value);
...
bool TryInitProjectFromEmbeddedTemplates(const std::filesystem::path& root,
    ...
    const auto config_path = root / "mblink.config.json";
    if (std::filesystem::exists(config_path)) {
        auto config_json = ReadJsonFile(config_path, error);
        if (!config_json || !config_json->is_object()) {
            if (error && error->empty()) *error = "模板 mblink.config.json 无效";
            return false;
        }
    ...
    if (result) {
        ...
        result->next_step = "执行 mblink-ui-dev open \"" + PathToUtf8(root) + "\"";
    }
    return true;
}
...
bool TryInitProjectFromEmbeddedTemplateLayers(const std::filesystem::path& root,
    ...
    if (std::filesystem::exists(config_path)) {
        auto config_json = ReadJsonFile(config_path, error);
        if (!config_json || !config_json->is_object()) {
            if (error && error->empty()) *error = "template mblink.config.json is invalid";
            return false;
        }
        (*config_json)["name"] = project_name;
    ...
    if (result) {
        ...
        result->files_created = files_created;
        result->next_step = "鎵ц mblink-ui-dev open \"" + PathToUtf8(root) + "\"";
    }
    return true;
}
...
std::filesystem::path GetProjectsDataRoot() {
    return std::filesystem::temp_directory_path() / "mblink-ui-dev" / "projects";
}
...
std::filesystem::path GetStateFilePath(const std::string& project_id) {
    if (project_id.empty()) return std::filesystem::temp_directory_path() / "mblink-ui-dev.state.json";
...
std::string GetDaemonPipeName(const std::string& project_id) {
    if (project_id.empty()) return "\\\\.\\pipe\\mblink-ui-dev";
    return "\\\\.\\pipe\\mblink-ui-dev-" + SanitizeIdComponent(project_id);
}
...
ProjectConfig LoadProjectConfig(const std::filesystem::path& project_root, std::string* error) {
    ...
    const auto config_path = project_root / "mblink.config.json";
    if (!std::filesystem::exists(config_path)) return cfg;

    try {
        std::ifstream ifs(config_path, std::ios::binary);
        if (!ifs) {
            if (error) *error = "读取 mblink.config.json 失败";
            return cfg;
        }
        std::stringstream ss;
        ss << ifs.rdbuf();
        const auto j = nlohmann::json::parse(ss.str());
        if (!j.is_object()) {
            if (error) *error = "mblink.config.json 必须是 JSON object";
            return cfg;
        }
        ...
        if (cfg.entry.empty()) {
            if (error) *error = "mblink.config.json 缺少 entry";
            return cfg;
        }
        ...
        if (cfg.title.empty()) cfg.title = cfg.name.empty() ? "MBlink UI Dev" : cfg.name;
        ...
        if (error) *error = std::string("解析 mblink.config.json 失败: ") + e.what();
        return cfg;
    }
}
...
bool InitProject(const std::filesystem::path& target_dir,
    ...
    const std::string project_name = root_name.empty() ? "mblink-app" : root_name;
