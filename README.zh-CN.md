...
MBink 是一个以 Windows 为主的桌面 UI 框架与工具链，目标是用更轻量的原生运行时、现代 JavaScript UI 和对 AI 友好的开发流程，来构建中小型桌面应用。

## MBink 是什么

- 以 `mbink.dll` 为核心的共享运行时
- 一个更贴近底层、便于手动理解的 `esm_loader.exe`
- 一个面向 AI 开发流程的 `mbink-ui-dev.exe`
- 目前已具备 Python 集成，仓库中也包含早期 Rust 和 Go 绑定
...
MBink 面向的是希望获得更小体积、更强可控性，以及更适合人类与 AI 协同开发流程的桌面应用场景。

## 当前 UI 兼容主线

MBink 目前的 JS UI 兼容主线，是围绕轻量化的官方 Preact ESM 模块链路展开的：
...
而且这些官方 Preact 模块不只是仓库里的示例依赖，它们已经作为内置运行时模块注册进 MBink 的运行时链路里了。所以当前已验证、最默认的写法就是直接使用 ESM 导入：
...
MBink 支持真实的 ESM `import` 风格 UI 代码。当前运行时和工具链可以处理：
...
- `mbink-ui-dev` 提供了 `open`、`build`、`snapshot`、`query`、`inspect`、`click`、`serve` 这一套实用工作流
- `mbink.dll` 作为运行时核心，在工具链和各语言绑定之间保持一致
- `esm_loader` 提供了一条不隐藏 DLL/运行时结构的手动入口
- 仓库里已经有应用壳、Todo、日志视图、原生控件、桌面布局等可运行示例

## 实际效果

下面这些图都来自 Windows 上真实 `mbink-ui-dev` 会话后的运行截图。
...
MBink 的设计目标之一，是让 AI 代理和人类开发者走同一条开发路径：

- shell / CLI 代理可以直接驱动 `mbink-ui-dev.exe`
- 支持 MCP 的客户端可以通过 `mbink-ui-dev.exe serve`
- 直接基于 C API 的宿主可以通过可选的 `mbink_devtools.dll` 暴露实时 UI 分析能力
...
- `mbink-ui-dev.exe`：项目工具、快照、构建/重载、MCP 和检查能力
- `mbink_devtools.dll`：开发时可选的 UI 快照、控制和运行时 HTTP MCP 伴随组件
...
- `todo_app_js`：最小、已验证、适合快速检查 `mbink-ui-dev`
- `modern_desktop_demo`：偏桌面应用风格的壳与布局示例
- `terminal_logview_demo`：MBink 原生 terminal / logview 元素示例
- `component_demo`：更小粒度的 UI 组件组合示例
- `official_preact_jsx_dev`：官方 Preact ESM 链路的参考示例

## 当前项目阶段

MBink 目前属于“早期但已经能看懂和跑起来”的仓库阶段：

- Windows 是证据最充分的平台
- 工具链和示例工作流比对外发布包装更成熟
- Python 是当前最适合先读的绑定
- Rust 和 Go 绑定已存在，但对外上手仍是次要路径
...
如果 MBink 对你有帮助，赞助可以直接支持这些事情继续推进：
...
tools/       mbink-ui-dev、esm_loader 以及构建工具
