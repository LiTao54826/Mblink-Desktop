/**
 * @file render_pipeline_legacy.h
 * @brief 旧版渲染管线（已废弃，保留用于过渡）
...
/**
 * @brief 渲染生命周期阶段（旧版）
 * @deprecated 请使用 RenderStage
 */
enum class RenderLifecycleLegacy {
    Idle,                    ///< 空闲状态
 ...
 * 确保渲染流程的正确顺序和状态一致性。
 */
class RenderPipelineLegacy {
public:
    RenderPipelineLegacy();
    ~RenderPipelineLegacy();

    // 禁止拷贝
    RenderPipelineLegacy(const RenderPipelineLegacy&) = delete;
    RenderPipelineLegacy& operator=(const RenderPipelineLegacy&) = delete;
     ...
     * @return 当前阶段
     */
    RenderLifecycleLegacy GetLifecycle() const { return lifecycle_; }
...
private:
    /// 当前生命周期阶段
    RenderLifecycleLegacy lifecycle_ = RenderLifecycleLegacy::Idle;
