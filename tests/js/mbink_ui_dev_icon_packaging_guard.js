// Regression check: mbink-ui-dev host builds must package configured Windows exe icons.
// Usage: node tests/js/mbink_ui_dev_icon_packaging_guard.js
...
function run() {
  const commonHeader = readRepoFile('tools', 'mbink_ui_dev', 'common.h');
  const commonSrc = readRepoFile('tools', 'mbink_ui_dev', 'common.cpp');
  const daemonSrc = readRepoFile('tools', 'mbink_ui_dev', 'daemon_server.cpp');
  ...
  assert(commonSrc.includes('{"icon", p.build_icon}') &&
    ...
    'ProjectConfig JSON and mbink.config.json loading must include build.icon');
  ...
  const purposeIconRoots = purposes.flatMap((purpose) =>
    runtimes.map((runtime) =>
      path.join(process.cwd(), 'tools', 'mbink_ui_dev', 'templates', 'purpose', purpose, 'runtime', runtime)
    )
  );
  ...
  const cliReference = readRepoFile(
    'tools', 'mbink_ui_dev', 'skills', 'mbink-ui-dev', 'references', 'cli-mcp-reference.md'
  );
  const compatibility = readRepoFile(
    'tools', 'mbink_ui_dev', 'skills', 'mbink-ui-dev', 'references', 'compatibility-guidelines.md'
  );
  assert(cliReference.includes('build.icon = "assets/app.ico"') &&
    ...
    'mbink-ui-dev docs must describe build.icon and skip semantics');

  console.log('[PASS] mbink-ui-dev icon packaging guard is present');
}
