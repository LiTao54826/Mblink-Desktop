// Regression check: mbink-ui-dev builds must preserve static media assets for
// audio/image elements that resolve paths relative to the JS entry.
// Usage: node tests/js/mbink_ui_dev_static_audio_assets_guard.js
...
function run() {
  ...
  assert(uiDevSrc.includes('CopyUiStaticAssets') &&
    ...
    'mbink-ui-dev must copy src/public/assets static resources into build outputs');
  ...
  const apiSrc = fs.readFileSync(
    path.join(process.cwd(), 'core', 'api', 'mbink.cpp'),
    'utf8'
  );
  assert(apiSrc.includes('SetRuntimeBasePath(ctx, next_base_path)') &&
    ...
    'mbink_load_js_file must set the document/fetch/image base path to the JS entry directory');
  ...
  console.log('[PASS] mbink-ui-dev static audio asset guard is present');
}
