...
$exe = Join-Path (Get-Location) 'build/bin/Release/mblink-ui-dev.exe'
$todo = Join-Path (Get-Location) 'examples/todo_app_js'
$canonicalDev = Join-Path (Get-Location) 'tmp/mblink_ui_dev_p0_canonical_template'
$projA = Join-Path (Get-Location) 'tmp/mblink_ui_dev_multi_verify_a'
$projB = Join-Path (Get-Location) 'tmp/mblink_ui_dev_multi_verify_b'
$tmpJs = Join-Path (Get-Location) 'tmp/mblink_ui_dev_eval_test.js'
if (!(Test-Path $exe)) { throw "missing exe: $exe" }

function Stop-TestRuntimeProcesses() {
    $titles = @('todo_app_js', 'mblink_ui_dev_p0_canonical_template', 'mblink_ui_dev_multi_verify_a', 'mblink_ui_dev_multi_verify_b')
    ...
    $daemonProcesses = @(Get-CimInstance Win32_Process |
        Where-Object {
            ...
            $_.Name -eq 'mblink-ui-dev.exe' -and
...
function Reset-MultiVerifyFixtures() {
    ...
    Write-Utf8File (Join-Path $projA 'mblink.config.json') @'
{
  "name": "mblink_ui_dev_multi_verify_a",
  "template": "vanilla-js",
  "entry": "src/app.js",
  "src_dir": "src",
  "out_dir": ".dist",
  "window": { "title": "mblink_ui_dev_multi_verify_a", "width": 1280, "height": 800 },
  "build": {
    "builder": "esbuild",
    "jsx_factory": "h",
...
'@
    ...
    Write-Utf8File (Join-Path $projB 'mblink.config.json') @'
{
  "name": "mblink_ui_dev_multi_verify_b",
  "template": "preact-jsx",
  "entry": "src/App.jsx",
  "src_dir": "src",
  "out_dir": ".dist",
  "window": { "title": "mblink_ui_dev_multi_verify_b", "width": 1280, "height": 800 },
  "build": {
    "builder": "esbuild",
    "jsx_factory": "h",
    "jsx_fragment": "Fragment",
...
try {
    ...
    Assert ($init.result.serverInfo.name -eq 'mblink-ui-dev') 'initialize failed'
...
Write-Host '[PASS] mblink-ui-dev P0 regression green'
