...
TEST_F(IncrementalLayoutIntegrationTest, FixedHeightBlockPassesContentBoxHeightToPercentChild) {
    auto root = createFixedBlock(800.0f, 600.0f);

    auto host = createFixedBlock(300.0f, 200.0f);
    auto host_style = host->GetComputedStyle();
    host_style.padding = CSSEdges(CSSLength(10.0f, CSSUnit::PX));
    host->SetComputedStyle(host_style);

    auto fill = createAutoBlock();
    auto fill_style = fill->GetComputedStyle();
    fill_style.width = CSSLength(100.0f, CSSUnit::PERCENT);
    fill_style.height = CSSLength(100.0f, CSSUnit::PERCENT);
    fill->SetComputedStyle(fill_style);
    ...
    EXPECT_FLOAT_EQ(host->GetLayoutInfo().width, 320.0f);
    EXPECT_FLOAT_EQ(host->GetLayoutInfo().height, 220.0f);
    EXPECT_FLOAT_EQ(fill->GetLayoutInfo().width, 300.0f);
    EXPECT_FLOAT_EQ(fill->GetLayoutInfo().height, 200.0f);
}

TEST_F(IncrementalLayoutIntegrationTest, GridItemDefiniteHeightPassesToPercentBlockChild) {
    auto root = createFixedBlock(400.0f, 400.0f);
    auto root_style = root->GetComputedStyle();
    root_style.display = RenderObjectType::GRID;
    root_style.grid_template_columns = "1fr";
    root_style.grid_template_rows = "1fr 1fr";
    root->SetComputedStyle(root_style);

    auto host_top = createAutoBlock();
    auto host_bottom = createAutoBlock();

    auto fill_top = createAutoBlock();
    auto fill_top_style = fill_top->GetComputedStyle();
    fill_top_style.width = CSSLength(100.0f, CSSUnit::PERCENT);
    fill_top_style.height = CSSLength(100.0f, CSSUnit::PERCENT);
    fill_top->SetComputedStyle(fill_top_style);

    auto fill_bottom = createAutoBlock();
    auto fill_bottom_style = fill_bottom->GetComputedStyle();
    fill_bottom_style.width = CSSLength(100.0f, CSSUnit::PERCENT);
    fill_bottom_style.height = CSSLength(100.0f, CSSUnit::PERCENT);
    fill_bottom->SetComputedStyle(fill_bottom_style);

    root->AppendChild(host_top);
    root->AppendChild(host_bottom);
    host_top->AppendChild(fill_top);
    host_bottom->AppendChild(fill_bottom);

    layout_engine_->BuildLayoutTree(root);
    layout_engine_->ComputeLayout(400.0f, 400.0f);
    layout_engine_->GetLayoutInfo(root);

    EXPECT_FLOAT_EQ(host_top->GetLayoutInfo().height, 200.0f);
    EXPECT_FLOAT_EQ(host_bottom->GetLayoutInfo().height, 200.0f);
    EXPECT_FLOAT_EQ(fill_top->GetLayoutInfo().height, 200.0f);
    EXPECT_FLOAT_EQ(fill_bottom->GetLayoutInfo().height, 200.0f);
}

TEST_F(IncrementalLayoutIntegrationTest, NestedGridPanelPassesDefiniteHeightToPercentBlockChild) {
    auto root = createFixedBlock(1000.0f, 700.0f);
    auto root_style = root->GetComputedStyle();
    root_style.display = RenderObjectType::GRID;
    root_style.grid_template_columns = "300px 1fr";
    root_style.padding = CSSEdges(CSSLength(12.0f, CSSUnit::PX));
    root->SetComputedStyle(root_style);

    auto left = createAutoBlock();
    auto left_style = left->GetComputedStyle();
    left_style.padding = CSSEdges(CSSLength(14.0f, CSSUnit::PX));
    left->SetComputedStyle(left_style);

    auto right = createAutoBlock();
    auto right_style = right->GetComputedStyle();
    right_style.display = RenderObjectType::GRID;
    right_style.grid_template_rows = "1fr 1fr";
    right_style.row_gap = CSSLength(12.0f, CSSUnit::PX);
    right->SetComputedStyle(right_style);

    auto host_top = createAutoBlock();
    auto host_top_style = host_top->GetComputedStyle();
    host_top_style.padding = CSSEdges(CSSLength(0.0f, CSSUnit::PX));
    host_top->SetComputedStyle(host_top_style);

    auto host_bottom = createAutoBlock();
    auto host_bottom_style = host_bottom->GetComputedStyle();
    host_bottom_style.padding = CSSEdges(CSSLength(0.0f, CSSUnit::PX));
    host_bottom->SetComputedStyle(host_bottom_style);
    ...
    right->AppendChild(host_top);
    right->AppendChild(host_bottom);
    host_top->AppendChild(fill_top);
    host_bottom->AppendChild(fill_bottom);
    ...
    EXPECT_GT(host_top->GetLayoutInfo().height, 0.0f);
    EXPECT_FLOAT_EQ(host_top->GetLayoutInfo().height, fill_top->GetLayoutInfo().height);
