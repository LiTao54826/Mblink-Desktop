...
MBink 仓库里自带了一套 repo-local skills / 技能书，用来让 AI 编码工具基于项目自己的工作流做事，而不是只看目录后自行猜测。

## 这是什么

MBink 项目工作流对应的技能书目录在这里：
...
当你希望 AI 工具按照 MBink 已验证过的开发路径工作时，就应该使用这套技能书。
...
- `supported-api-reference.md`：当前 MBink UI 工作可依赖的 API 范围
...
1. 打开 MBink 仓库。
2. 让代理使用 `mbink-ui-dev` skill。
3. 在修改代码前先读取 `tools/mbink_ui_dev/skills/mbink-ui-dev/SKILL.md`。
4. 优先通过 `mbink-ui-dev.exe` 进行项目工作。
...
1. 直接阅读 `tools/mbink_ui_dev/skills/mbink-ui-dev/SKILL.md`。
2. 把这个文件路径告诉你的代理，或者把相关内容贴进上下文。
3. 按照同样的 `mbink-ui-dev.exe` CLI 工作流执行。

## 它和 MBink 运行时分层是什么关系

技能书描述的是工作流，不是另一套运行时。

- `mbink-ui-dev.exe`：项目级工作入口，负责 open/build/snapshot/query/inspect/MCP
- `mbink.dll`：运行时核心
- `mbink_devtools.dll`：开发期可选伴随组件，用于实时快照/控制和运行时 HTTP MCP
- `esm_loader.exe`：围绕 `mbink.dll` 的最薄手动宿主
...
- MBink 的 skills / 技能书到底放在哪里
- 这套技能书包含哪些文件
- skill-aware AI 工具应该怎样进入这个仓库
- `mbink-ui-dev`、`mbink.dll`、`mbink_devtools.dll`、`esm_loader` 之间是什么关系
